...
@section('main-content')
    ...
    <h3 class="text-center my-3">Available <span class="text-info">Projects</span></h3>
                    ...
                    <tbody>
                        ...
                        @foreach ($projects as $project)
                            <tr>
                                ...
                                <td><a href="{{ route('view.project', ['id' => $project->id]) }}" class="nav-link">
            ...
            <div class="accordion accordion-flush" id="accordionFlushExample">
                <div class="accordion-item">
                    <h2 class="accordion-header" id="flush-headingOne">
                        <button class="accordion-button collapsed btn btn-danger text-light btn-block" type="button"
                                ...
                                @foreach ($users as $user)
                                    <div class="card w-100 mb-md-0 mb-3">
                                        <div class="card-header bg-dark text-light">
                                            ...
                                            <small class="float-right">
                                                @if ($user->user_role == 'developer')
                                                    <span
                                                        class="badge-success text-capitalize p-1">{{ $user->user_role }}</span>
                                                @endif

                                                @if ($user->user_role == 'client')
                                                    <span
                                                        class="badge-info text-capitalize p-1">{{ $user->user_role }}</span>
                                                @endif
                                            </small>
                                        </div>
                                        <div class="card-body">
                                        ...
                                        <div class="card-footer bg-warning text-light">
                                            ...
                                            <a class="float-right">
                                                <form action="{{ route('admin.delete.user', ['id' => $user->id]) }}"
                                                    method="post">
                                                    @csrf
                                                    <button type="submit" class="btn btn-default mt-n2"><i
                                                            class="bi text-danger text-sm bi-trash3"></i></button>
                                                </form>
                                            </a>
                                            <a href="{{ route('admin.edit.user', ['id' => $user->id]) }}"
                                                class="float-right"><i
                                                    class="bi bi-pencil-square text-primary text-sm mx-3"></i></a>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
