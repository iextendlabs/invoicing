    ...
    <div class="row">
        <div class="col-md-8 offset-md-2 col-sm-12">
            <form action="{{ route('store.task.log') }}" method="post">
                ...
                <input type="text" class="form-control form-control-sm" id="date_creation"
                    value="{{ old('date_creation') }}" placeholder="Enter Date" name="date_creation"
                    onfocus="(this.type='date')" id="">
                @if ($errors->any())
                ...
                <div class="row">
                    <div class="col-6">
                        ...
                        <input type="time" placeholder="MM/DD/YYYY" class="form-control" value="{{ old('starttime') }}"
                            name="starttime">
                        @if ($errors->any())
                ...
                <button type="submit" class="btn btn-default btn-block border-danger text-primary mb-5">Create Log</button>
            </form>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            $('#project_id').change(function() {
                ...
                if (projectId) {
                    $.ajax({
                        url: "{{ route('tasks.by.project', ':project_id') }}".replace(
                        ...
                        success: function(data) {
                            console.log(data);
                            $('#task_id').empty();
                            $('#task_id').append('<option value="">Select a task</option>');
                            $.each(data, function(key, value) {
                                $('#task_id').append('<option value="' + value.id +
                                    '">' + value.task_title + '</option>');
                            });
                        }
                    }); 
